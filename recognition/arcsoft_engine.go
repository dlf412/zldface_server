...
func bytesToNative(arr []byte) *C.ASF_FaceFeature {
	var native = &C.ASF_FaceFeature{}
	length := len(arr)
	native.featureSize = C.int(length)

	addr := (*C.MByte)(C.malloc(C.size_t(length)))
	ps := (*[1 << 28]C.MByte)(unsafe.Pointer(addr))[:length:length]
...
func (engine *FaceEngine) FaceFeatureCompareEx(arr1, arr2 []byte) (float32, error) {
	var confidenceLevel float32 = 0
